---
layout: default
title: About
nav_exclude: true 
---

<div style="display: flex; align-items: flex-start; gap: 20px;">
  <img class ="profile-pic" src="/assets/Nithya G.png" alt="Nithya Giri profile photo" style="width: 300px; flex-shrink: 0;" >
  <div>
  <p>
I’m a motivated software developer with hands-on experience building user-friendly web applications. I enjoy turning ideas into functional, visually engaging products and continuously improving my technical skills through real-world projects.
</p>

<h3> Tech Stack & Skills</h3>    
<p><strong>Languages:</strong> JavaScript, Java, SQL, HTML, CSS</p>
<p><strong>Tools:</strong>Git/GitHub, VS Code, IntelliJ IDEA, Postman, Chrome DevTools</p>
<p><strong>Frameworks/Libraries:</strong> React, Node.js, SpringBoot</p>
</div>
  </div>


## What I do:

Build responsive web interfaces using HTML, CSS, and JavaScript

Create dynamic applications with React

Work with SQL to design and query databases

Apply Java and object-oriented programming concepts

Work with Springboot

Collaborate using Git and GitHub
        

##  What I’m Looking For
I’m seeking an **entry-level software developer or frontend developer role** where I can contribute to meaningful projects, learn from experienced teammates, and grow as a developer.

<p markdown="0">👉 Check out my <a href="/project/"><strong>Projects</strong></a> | <a href="/resume/"><strong>Resume</strong></a> | <a href="/contact/"><strong>Contact</strong></a> sections to learn more!
</p>